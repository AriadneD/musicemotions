...
function Analyze() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await saveSongAnalysis(user.uid, result);
      setSaved(true);
    } catch (err) {
      ...
      <main className="analyze-main">
        ...
        <AnimatePresence>
          {loading && (
            ...
            >
              ...
              <p className="loading-subtext">Downloading audio, extracting features, generating aura...</p>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {result && !loading && (
            ...
            >
              {/* Song Header with Aura Image */}
              <div className="song-header">
                <div className="thumbnail-wrapper">
                  {result.auraImage ? (
                    <img src={result.auraImage} alt="Song Aura" className="thumbnail aura-image" />
                  ...
                  {result.auraImage && <span className="aura-badge">✨ AI Aura</span>}
                </div>
              ...
              {activeTab === 'aura' && (
                ...
                >
                  <div className="aura-section">
                    <h3 className="section-title">Song Aura Visualization</h3>
                    <p className="aura-description">
                      An AI-generated abstract visualization of this song's emotional energy and character.
                    </p>
                    
                    {result.auraImage ? (
                      <div className="aura-display">
                        <motion.img 
                          src={result.auraImage} 
                          alt="Song Aura" 
                          className="aura-full"
                        ...
                        />
                        <button 
                          className="download-aura-btn"
                          onClick={() => {
                            const link = document.createElement('a');
                            link.href = result.auraImage;
                            const songName = result.metadata.title.replace(/[^a-z0-9]/gi, '_').toLowerCase();
                            link.download = `${songName}_aura.png`;
                            document.body.appendChild(link);
                            link.click();
                            document.body.removeChild(link);
                          }}
                        >
                          ⬇️ Download Aura Image
                        </button>
                        <div className="aura-info">
                          <div className="aura-stat">
                            <span className="aura-stat-label">Dominant Energy</span>
                            <span className="aura-stat-value">
                              {result.profile.energy > 0.6 ? 'High Intensity' : 
                               result.profile.energy < 0.4 ? 'Calm & Serene' : 'Balanced Flow'}
                            </span>
                          </div>
                          <div className="aura-stat">
                            <span className="aura-stat-label">Emotional Warmth</span>
                            <span className="aura-stat-value">
                              {result.profile.warmth > 0.6 ? 'Warm & Inviting' : 
                               result.profile.warmth < 0.4 ? 'Cool & Ethereal' : 'Neutral Tones'}
                            </span>
                          </div>
                          <div className="aura-stat">
                            <span className="aura-stat-label">Complexity</span>
                            <span className="aura-stat-value">
                              {result.profile.complexity > 0.6 ? 'Intricate Layers' : 
                               result.profile.complexity < 0.4 ? 'Clean & Simple' : 'Moderate Depth'}
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="aura-placeholder">
                        ...
                        <p>Aura visualization not available for this track.</p>
                      </div>
                    )}
                    
                    {ctx?.fun_insights?.if_this_song_were && (
                      <div className="aura-synesthesia">
                        <h4>Synesthetic Interpretation</h4>
                        <div className="synesthesia-mini-grid">
                          <div className="syn-mini">
                            <span className="syn-mini-label">Color</span>
                            <span className="syn-mini-value">{ctx.fun_insights.if_this_song_were.color}</span>
                          </div>
                          <div className="syn-mini">
                            <span className="syn-mini-label">Texture</span>
                            <span className="syn-mini-value">{ctx.fun_insights.if_this_song_were.texture}</span>
                          </div>
                          <div className="syn-mini">
                            <span className="syn-mini-label">Era</span>
                            <span className="syn-mini-value">{ctx.fun_insights.if_this_song_were.time_period}</span>
                          </div>
                        </div>
                      </div>
                    )}
              ...
              {activeTab === 'psychology' && ctx?.listener_psychology && (
                <motion.div className="tab-content" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                    ...
                    {ctx?.demographics && (
                      <>
                        ...
                        <div className="demographics-grid">
                          <div className="demo-item"><span className="demo-label">Age Range</span><span className="demo-value">{ctx.demographics.age_range}</span></div>
                          <div className="demo-item"><span className="demo-label">Lifestyle</span><span className="demo-value">{ctx.demographics.lifestyle}</span></div>
                          <div className="demo-item"><span className="demo-label">Aesthetic</span><span className="demo-value">{ctx.demographics.aesthetic}</span></div>
                          <div className="demo-item"><span className="demo-label">Subculture</span><span className="demo-value">{ctx.demographics.subculture}</span></div>
                        </div>
                      </>
                    )}
                  </div>
